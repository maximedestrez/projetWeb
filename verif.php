...
if (isset($_POST['email']) && isset($_POST['password'])) {
    ...
    $requete = "SELECT id, password, prenom, est_vendeur, solde FROM utilisateur WHERE email = ?";
    $stmt = mysqli_prepare($idcom, $requete);
    mysqli_stmt_bind_param($stmt, "s", $email);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_bind_result($stmt, $id_user, $password_hache, $prenom, $est_vendeur, $solde);
